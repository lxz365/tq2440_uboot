...
int do_help (cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (argc == 1) {	/*show list of commands */
		...
		cmd_start = (int)&__u_boot_cmd_start;
		cmd_end = (int)&__u_boot_cmd_end;		
		cmd_items = (cmd_end - cmd_start) / 24;
		//---------------------------------<
...
{
	cmd_tbl_t *cmdtp;
	cmd_tbl_t *cmdtp_temp = (cmd_tbl_t *)&__u_boot_cmd_start;	/*Init value */
	const char *p;
	int len;
	int n_found = 0;
	...
	for (cmdtp = (cmd_tbl_t *)&__u_boot_cmd_start;
	     cmdtp != (cmd_tbl_t *)&__u_boot_cmd_end;
	     cmdtp++) {
